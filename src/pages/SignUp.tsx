...
const demoAccounts: Record<DemoType, DemoConfig> = {
  learner: { email: "demo@learner.com", password: "demo123", name: "Demo Learner" },
  mentor: { email: "demo@mentor.com", password: "mentor123", name: "Demo Mentor" },
};
...
const SignUp = () => {
  ...
  const { signup, user, isAuthLoading, signInWithGoogle } = useAuth();
  ...
  const handleDemoSignup = async (type: DemoType) => {
    const data = demoAccounts[type];
    setIsLoading(true);
    try {
      await signup(data.email, data.password, data.name);
      navigate("/dashboard");
    } catch (error) {
      const message = error instanceof Error ? error.message : "Failed to create demo account. Please try again.";
      setErrors({ submit: message });
    } finally {
      setIsLoading(false);
    }
  ...
  const confirmPasswordType = useMemo(() => (showConfirmPassword ? "text" : "password"), [showConfirmPassword]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-col gap-10 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="space-y-5">
            <div className="flex flex-col items-center gap-4 lg:items-start">
              ...
              <div className="max-w-md space-y-3">
                <h1 className="text-3xl font-bold leading-snug text-white">Create Your Account</h1>
                <p className="text-base text-slate-300">Join the EduCareer AI learning community</p>
              </div>
            </div>
          </div>
        </motion.section>
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:w-1/2">
          <Card className="bg-white/85 dark:bg-slate-900/60 backdrop-blur-xl border border-border shadow-2xl">
            ...
            <CardContent className="space-y-3 pt-2 pb-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <Button
                  type="submit"
                  className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-primary-foreground font-semibold py-2"
                  disabled={isLoading}
                >
              ...
              <div className="space-y-2.5 pt-1">
                ...
                <Button
                  type="button"
                  variant="outline"
                  className="w-full gap-2 border-border/80 bg-white/85 text-foreground hover:bg-primary/10 dark:bg-slate-900/70 py-2"
          ...
          >
            By creating an account, you agree to the <Link to="/terms" className="text-primary hover:underline">EduCareer Terms of Service</Link> and <Link to="/privacy" className="text-primary hover:underline">Privacy Policy</Link>
          </motion.p>
        </motion.section>
      </div>
    </div>
