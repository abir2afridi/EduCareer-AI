      ...
      <Card className="relative overflow-hidden border border-border/40 bg-white/95 shadow-lg shadow-primary/5 dark:bg-slate-950/85">
        ...
        <CardContent className="relative z-10 space-y-8 pb-8">
          <section className="space-y-5">
            <div className="group relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-background via-background/85 to-background/65 p-5 shadow-md transition-all duration-500 hover:border-primary/35 hover:shadow-primary/15">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(59,130,246,0.16),transparent_55%)] opacity-75 transition-opacity duration-500 group-hover:opacity-100" aria-hidden="true" />
              <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div className="space-y-2">
                  <Badge variant="outline" className="w-fit gap-1 rounded-full border-primary/40 bg-white/70 px-3 py-1 text-xs font-medium uppercase tracking-[0.3em] text-foreground shadow-sm">
                    <BookOpenCheck className="h-3.5 w-3.5" /> Course plan overview
                  </Badge>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Sparkles className="h-4 w-4" />
                    <p className="text-xs uppercase tracking-[0.25em]">University curated journey</p>
                  </div>
                  <h1 className="text-2xl font-semibold tracking-tight text-foreground lg:text-3xl">{heroTitle}</h1>
                  <p className="max-w-xl text-sm text-muted-foreground">{heroDescription}</p>
                  {selectedPublishedEntry ? (
                    <p className="text-xs text-muted-foreground">Last published update: {selectedPublishedEntry.updatedAt}</p>
                  ) : lastUpdated ? (
                    <p className="text-xs text-muted-foreground">Last sync: {lastUpdated}</p>
                  ) : derivedUniversity ? (
                    <p className="text-xs text-muted-foreground">Waiting for the latest publish from this university.</p>
                  ) : null}
                </div>
                <div className="flex flex-col items-start gap-4 md:flex-row md:items-center md:gap-6 lg:flex-col lg:items-end">
                  <div className="h-24 w-24 rounded-3xl border border-primary/30 bg-white/80 p-1.5 shadow-md backdrop-blur-sm transition-transform duration-500 group-hover:scale-105 md:h-28 md:w-28">
                    <DotLottieReact
                  ...
                  {heroLogo ? (
                    <img
                      src={heroLogo}
                      alt="University logo"
                      className="h-16 w-16 rounded-2xl border border-border/60 bg-white object-contain shadow-sm"
                    />
                  ) : (
                    <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-dashed border-border/60 text-muted-foreground">
                      <GraduationCap className="h-6 w-6" />
                    </div>
                  )}
                  <Badge variant="secondary" className="gap-2 rounded-full bg-primary/10 text-primary">
                    {derivedUniversity && (isLoadingPublished || isLoadingCompletions) ? <Loader2 className="h-3.5 w-3.5 animate-spin text-primary" /> : null}
                    {derivedUniversity
          ...
          <section className="space-y-5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Button variant="ghost" size="sm" onClick={clearSelections} disabled={!selectedUniversity && !publishedSelection}>
                Reset selection
              </Button>
            </div>
          </section>
        ...
        <TabsContent value="plan" className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {quickStats.map(({ label, value, hint, icon: Icon }) => (
              <div
                key={label}
                className="flex items-start gap-3 rounded-2xl border border-border/50 bg-white/70 p-4 shadow-sm backdrop-blur"
              >
